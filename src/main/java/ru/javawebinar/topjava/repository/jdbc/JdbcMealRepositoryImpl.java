...
public class JdbcMealRepositoryImpl implements MealRepository {
    ...
    public Meal save(Meal meal, int userId) {
        ...
        if (meal.isNew()) {
            Number newKey = insertMeal.executeAndReturnKey(sqlMap);
            meal.setId(newKey.intValue());
        } else if (namedParameterJdbcTemplate.update(
                "UPDATE meals SET dateTime=:dateTime, description=:description, calories=:calories, user_id=:user_id WHERE id=:id", sqlMap) == 0) {
            return null;
        }
        return meal;
    }
